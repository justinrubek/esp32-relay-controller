...
pub enum PageState {
    /// Storage is empty and not used to store data
    Uninitialized,
    /// Storage is initialized with page header and page sequence number
    Active,
    /// Storage is in a consistent state with key-value pairs, adding new values is impossible
    Full,
    /// Non-erased key-value pairs are being moved so this page can be erased
    Erasing,
    /// Page header contains invalid data. Any items are not accessible
    Corrupt,
...
#[repr(C)]
#[derive(AsBytes, Clone, Copy, Debug, FromBytes, Unaligned)]
pub struct RawPageState(U32<LittleEndian>);
impl RawPageState {
    const UNINITIALIZED: u32 = 0xffff_ffff;
    const ACTIVE: u32 = Self::UNINITIALIZED & !1;
    const FULL: u32 = Self::ACTIVE & !2;
    const ERASING: u32 = Self::FULL & !4;
    const CORRUPT: u32 = Self::ERASING & !8;
    const INVALID: u32 = 0;
...
impl From<PageState> for RawPageState {
    fn from(from: PageState) -> Self {
        let value = match from {
            ...
            PageState::Erasing => Self::ERASING,
...
impl TryFrom<RawPageState> for PageState {
    ...
    fn try_from(value: RawPageState) -> Result<Self, Self::Error> {
        match value.0.get() {
            ...
            RawPageState::ERASING => Ok(Self::Erasing),
...
impl EntryStateBitmap {
    ...
    /// # Panics
    /// - the provided index is greater than 126
    #[must_use]
...
impl From<EntryType> for RawEntryType {
    fn from(value: EntryType) -> Self {
        match value {
            ...
            EntryType::String => Self(0x21),
            EntryType::BlobData => Self(0x42),
            EntryType::BlobIndex => Self(0x48),
...
impl TryFrom<RawEntryType> for EntryType {
    ...
    fn try_from(value: RawEntryType) -> Result<Self, Self::Error> {
        match value.0 {
            ...
            0x21 => Ok(EntryType::String),
            0x42 => Ok(EntryType::BlobData),
            0x48 => Ok(EntryType::BlobIndex),
...
define_to_data!(u8, U8);
define_to_data!(i8, I8);
define_to_data!(u16, U16);
define_to_data!(i16, I16);
define_to_data!(u32, U32);
define_to_data!(i32, I32);
define_to_data!(u64, U64);
define_to_data!(i64, I64);
...
impl Entry {
    ...
    pub fn update_crc(&mut self) {
        ...
        let new_crc = calculate_crc([&bytes[0..4], &bytes[8..]]);
        self.crc32.set(new_crc);
    }

    /// # Panics
    /// - the provided index is <= 0
    #[must_use]
    pub fn new_namespace(namespace: &Key, index: u8) -> Self {
        assert!(index > 0);
        let mut entry = Self {
    ...
    /// # Panics
    /// - the provided `namespace_index` is <= 0
    #[must_use]
    pub fn new_primitive<T: ToData>(namespace_index: u8, key: &Key, value: T) -> Self {
    ...
    /// - the provided `namespace_index` is <= 0
    #[must_use]
    pub fn new_string_header(namespace_index: u8, key: &Key, span: u8, data: &[u8]) -> Self {
        assert!(namespace_index > 0);
        let crc32 = calculate_crc([data, &[0x00]]);
        let mut entry_data = [0xff; 8];
        LittleEndian::write_u16(
            &mut entry_data[0..2],
            u16::try_from(data.len() + 1).expect("data length too large"),
        );
        LittleEndian::write_u32(&mut entry_data[4..8], crc32);

        let mut entry = Self {
            ns: namespace_index,
            r#type: EntryType::String.into(),
            span,
            chunk_index: 0xff,
            key: [0; 16],
            data: entry_data,
    ...
    /// # Panics
    /// - the provided `namespace_index` <= 0
    #[must_use]
    pub fn new_blob_data(
        ...
        LittleEndian::write_u16(
            &mut entry_data[0..2],
            u16::try_from(chunk_data.len()).expect("chunk data length too large"),
        );
        LittleEndian::write_u32(&mut entry_data[4..8], crc32);
    ...
    pub fn new_blob_index(
        ...
        let mut entry = Self {
            ns: namespace_index,
            r#type: EntryType::BlobIndex.into(),
            span: 1,
            chunk_index: 0xff,
            key: [0; 16],
            data: entry_data,
...
pub type Key = heapless::String<15>;

pub struct Partition<const NUMBER_OF_ENTRIES_IN_PAGE: usize = { 4096 / 32 - 2 }> {
    namespaces: HashMap<Key, u8>,
    pages: Vec<Page<NUMBER_OF_ENTRIES_IN_PAGE>>,
}

impl<const NUMBER_OF_ENTRIES: usize> Default for Partition<NUMBER_OF_ENTRIES> {
    fn default() -> Self {
        Self::new()
    }
}
impl<const NUMBER_OF_ENTRIES: usize> Partition<NUMBER_OF_ENTRIES> {
    ...
    fn new_page(&mut self) -> &mut Page<NUMBER_OF_ENTRIES> {
        ...
        page.header.sequence_no = U32::new(u32::try_from(num_pages).expect("too many pages"));
        page.header.state = PageState::Active.into();
        page
    }

    fn num_remaining_entries(&self) -> usize {
        self.pages.last().map_or(0, Page::num_remaining_entries)
    }

    fn get_or_add_namespace(&mut self, namespace: &Key) -> u8 {
        if let Some(index) = self.namespaces.get(namespace) {
            *index
        } else {
            let new_index = u8::try_from(self.namespaces.len() + 1).expect("too many namespaces");
    ...
    pub fn add_primitive_entry<T: ToData>(&mut self, namespace: &Key, key: &Key, value: T) {
    ...
    /// # Errors
    /// - the provided `value` string is longer than `(NUMBER_OF_ENTRIES - 1) * Entry::SIZE` - 1, which is `(126 - 1) * 32 - 1 == 3999`
    ///
    /// # Panics
    /// - if the number of required entries exceeds 255
    pub fn add_string_entry(
        ...
        let value_len = value_bytes.len() + 1; // +1 for null terminator
        ...
        let entry = Entry::new_string_header(
            namespace_index,
            key,
            u8::try_from(num_required_entries).expect("too many required entries"),
            value_bytes,
        );
        self.add_entry_or_data(EntryOrData::Entry(entry));
    ...
    ///
    /// # Panics
    /// - the data is too large TODO: improve the wording
    pub fn add_binary_entry(
        ...
        while bytes_written < value_len {
            ...
            let entry = Entry::new_blob_data(
                namespace_index,
                key,
                u8::try_from(span).expect("span too large"),
                num_chunks,
                chunk_data,
            );
            self.add_entry_or_data(EntryOrData::Entry(entry));
        ...
        let index_entry = Entry::new_blob_index(
            namespace_index,
            key,
            0,
            num_chunks,
            u32::try_from(value_len).expect("value length too large"),
        );
    ...
    fn finalize(&mut self) {
        if let Some(last_page_idx) = self.pages.len().checked_sub(1) {
            for (i, page) in self.pages.iter_mut().enumerate() {
                if i == last_page_idx {
                    // the last page should remain Active if it's the current working page
                    page.header.state = PageState::Active.into();
                } else {
...
impl EncryptionKey {
    ...
    /// # Panics
    /// - failure to fill the key buffer with random data
    #[must_use]
    ...
    pub fn from_reader<R: std::io::Read>(&mut self, reader: &mut R) -> std::io::Result<()> {
...
mod test {
    ...
    fn test_namespace_entry() {
        let entry = Entry::new_namespace(&Key::from_str("hoge").unwrap(), 1);
    ...
    fn test_primitive_entry_u8() {
        let entry = Entry::new_primitive(0xaa, &Key::from_str("hoge").unwrap(), 0xa5u8);
        let bytes = entry.as_bytes();
    ...
    fn test_primitive_entry_i8() {
        let entry = Entry::new_primitive(0xaa, &Key::from_str("hoge").unwrap(), -2i8);
        let bytes = entry.as_bytes();
    ...
    fn test_primitive_entry_u16() {
        let entry = Entry::new_primitive(0xaa, &Key::from_str("hoge").unwrap(), 0xdeadu16);
        let bytes = entry.as_bytes();
    ...
    fn test_primitive_entry_i16() {
        let entry = Entry::new_primitive(0xaa, &Key::from_str("hoge").unwrap(), -32768i16);
        let bytes = entry.as_bytes();
    ...
    fn test_primitive_entry_u32() {
        let entry = Entry::new_primitive(0xaa, &Key::from_str("hoge").unwrap(), 0xdead_beefu32);
        let bytes = entry.as_bytes();
    ...
    fn test_primitive_entry_i32() {
        let entry = Entry::new_primitive(0xaa, &Key::from_str("hoge").unwrap(), -0x8000_0000i32);
        let bytes = entry.as_bytes();
    ...
    fn test_primitive_entry_u64() {
        let entry = Entry::new_primitive(
            0xaa,
            &Key::from_str("hoge").unwrap(),
            0xdead_beef_cafe_aa55u64,
        );
        let bytes = entry.as_bytes();
    ...
    fn test_entry_str() {
        ...
        let entry = Entry::new_string_header(
            0xbb,
            &Key::from_str("0123456789abcde").unwrap(),
            0x7f,
            &data,
        );
        let bytes = entry.as_bytes();
    ...
    fn test_page_header() {
        ...
        assert_eq!(&header.as_bytes()[0..4], 0xffff_fffe_u32.as_bytes());
        header.state = PageState::Full.into();

        assert_eq!(&header.as_bytes()[0..4], 0xffff_fffc_u32.as_bytes());
        header.state = PageState::Erasing.into();

        assert_eq!(&header.as_bytes()[0..4], 0xffff_fff8_u32.as_bytes());
        header.state = PageState::Corrupt.into();

        assert_eq!(&header.as_bytes()[0..4], 0xffff_fff0_u32.as_bytes());
        header.state = PageState::Invalid.into();

        assert_eq!(&header.as_bytes()[0..4], 0x0000_0000_u32.as_bytes());
        header.sequence_no = U32::new(0xdead_beef);

        assert_eq!(&header.as_bytes()[4..8], 0xdead_beef_u32.as_bytes());
    ...
    fn test_encryption_key() {
        ...
        key.to_writer(&mut buffer).unwrap();
        assert_eq!(buffer.len(), 4096);

        let mut reader = std::io::Cursor::new(&buffer);
        let mut key_imported = EncryptionKey::new([0; 64]);
        key_imported.from_reader(&mut reader).unwrap();
